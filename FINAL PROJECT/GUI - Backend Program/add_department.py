...
def main2():
    ...
    def main_interface():
        ...
        text_reports = tk.Label(top_frame, text="Add Department", font=("Helvetica", 16), bg='#333333', fg='#ffffff')
    ...
    def load_department_data():
        ...
        if department_info is not None:
            ...
            custom_show_error("DATA ALREADY EXISTING!")

        else:
            # Handle the case where no data is found for the given em_id
            custom_show_error("NO DATA AVAILABLE.\n\nYOU MAY INSERT NEW DATA")
    ...
    def input_boxes():
        ...
        checkforduplicate_text = tk.Label(window, text = "Input A ID Number #301 & Up - Checking for Duplicates.", font=("Helvetica", 9, "bold"))
        checkforduplicate_text.place(relx=0.45 + x_axis, rely=0.32 + y_axis, anchor=tk.CENTER)
        # Employee ID
        add_em_button = tk.Button(window, text="Add Department", fg="white", height=2, width=20, command=add_department_boxbg, background="black")
        add_em_button.pack()
        add_em_button.place(x=980, y=180)

        # Job ID
        add_jobid_text = tk.Label(window, text="Add Job ID: ", font=("Helvetica", 12, "bold"))
    ...
    def add_department_boxbg():
        ...
        db.insert_department(add_jobid_val, add_jobdpt_val, add_jobname_val, add_jobdesc_val, add_salaryrange_val)
        custom_show_error("NEW DEPARTMENT ADDED!")
    ...
    def editprofile_box():
        ...
        big_text = tk.Label(window, text="Add Department", font=("Helvetica", 36, "bold"), bg="lightgrey")
